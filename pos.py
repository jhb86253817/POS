...
def read_data(filename):
    ...
    features = [(pos[i][0],pos[i][1]) for i in xrange(1,len(pos)-1) if pos[i][0]!='<s>']
    contexts = [feature[0] for feature in features]
    features_ins_num = len(features)
    ...
    e_q = \
    [p_tag_given_context(features_list[i],weights,features_list,features_set)*contexts_count[features_list[i][0]]/features_ins_num
